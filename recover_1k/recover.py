...
def main_worker(gpu, ngpus_per_node, args, model_teacher, model_verifier, ipc_id_range):
    ...
    if not load_tag:
        train_dataset = dataset_lt_data.LT_Dataset(args.train_data_path, '',transform=transforms.Compose([
                                                         transforms.Resize((224, 224)),
                                                         # transforms.RandomHorizontalFlip(),
                                                         transforms.ToTensor(),
                                                         transforms.Normalize(mean=[0.485, 0.456, 0.406],
...
def main_syn():
    ...
    parser.add_argument('--statistic-path', type=str, default='./statistic_test',
