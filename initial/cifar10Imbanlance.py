...
class Cifar10Imbanlance(Dataset):
    ...
    def produce_imbanlance_data(self, imbanlance_rate, file_path="/data", train=True):
        ...
        for i in range(1, self.num_cls + 1):
            ...
            fill_num = max_count - tem_data.shape[0]
            if fill_num > 0:
                H, W, C = tem_data.shape[1], tem_data.shape[2], tem_data.shape[3]
                # np.zeros => 黑图；dtype 保持一致
                blank_imgs = np.zeros((fill_num, H, W, C), dtype=tem_data.dtype)
                blank_labels = np.full(fill_num, i-1, dtype=np.int64)
                tem_data = np.concatenate([tem_data, blank_imgs], axis=0)
                tem_label = np.concatenate([tem_label, blank_labels], axis=0)
