...
def get_args():
    ...
    parser.add_argument('--ema-dr', default=0.99,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.model == 'resnet':
        model = Resnet_LT2.resnext50_32x4d(num_classes=1000)
    else:
    ...
    args.aux_teacher = ["wide_resnet50_2"]
    model_teacher = []
    net = Resnet_LT.resnext50_32x4d(num_classes=1000)
    model_teacher.append(net)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # print(args.best_epoch,args.best_acc1)
        save_checkpoint({
            'epoch': epoch + 1,
...
def train(model, model_teacher, args, epoch=None, gpu=0, ngpus_per_node=1, scaler=None, ema_model=None):
    ...
    for batch_idx, (images, target) in enumerate(args.train_loader):
        ...
        if args.loss_type == "kl":
            output = output / output.std(dim=1, keepdim=True)
            soft_label = soft_label / soft_label.std(dim=1, keepdim=True)
            output = F.log_softmax(output / args.temperature, dim=1)
            soft_label = F.softmax(soft_label / args.temperature, dim=1)
            loss = loss_function_kl(output, soft_label)+ F.cross_entropy(output, target) * args.ce_weight 
            ...
            # soft_label_norm = F.normalize(soft_label, p=2, dim=1)
            # output = output / output.std(dim=1, keepdim=True)
            # soft_label = soft_label / soft_label.std(dim=1, keepdim=True)
