...
class BNFeatureHook():
    def __init__(self, module, save_path="./", training_momentum=0.4, name=None, gpu=0, flatness_weight=0,
        ...
        self.category_save_path_list = [
            os.path.join(save_path, f"BNFeatureHook", f"class_{i}", name, "running.npz") for i in range(200)
        ]
        self.category_running_dd_var_list = [0. for i in range(200)]
        self.category_running_dd_mean_list = [0. for i in range(200)]
        self.load_tag = True
        ...
        if category_aware == "global":
            for i, category_save_path in enumerate(self.category_save_path_list):
                if os.path.exists(category_save_path):
                    ...
                    self.load_tag = False
                    self.counter = [0 for i in range(200)]
            if self.load_tag:
...
class ConvFeatureHook():
    def __init__(self, module=None, save_path="./", data_number=100000, name=None, gpu=0, training_momentum=0.4,
        ...
        if category_aware == "global":
            self.category_save_path_list = [
                os.path.join(save_path, f"ConvFeatureHook", f"class_{i}", name, "running.npz") for i in range(200)
            ]
            for i in range(200):
            ...
            self.category_running_dd_var_list = [0. for i in range(200)]
            self.category_running_dd_mean_list = [0. for i in range(200)]
            self.category_running_patch_var_list = [0. for i in range(200)]
            self.category_running_patch_mean_list = [0. for i in range(200)]
...
class PreImgPathCache(ImageFolder):
    def __init__(self, root, transforms):
        super(PreImgPathCache, self).__init__(root, transform=transforms)
        self.label2img = [[] for _ in range(len(self.classes))]
        self.label_ptr = [0 for _ in range(len(self.classes))]  # 👈 每类维护一个指针

        for img_path, label in self.samples:  # better than self.imgs
            label_idx = label  # in ImageFolder, label is already integer index from 0 to N-1
            self.label2img[label_idx].append(img_path)
