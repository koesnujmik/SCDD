...
import collections
import time
import ResNet_tiny,ResNet_cifar
...
def main_worker(gpu, ngpus_per_node, args, model_teacher, model_verifier, ipc_id_range):
    ...
    for zz in range(0, total_number, batch_size):
        ...
        if args.category_aware == "local":
            expand_ratio = int(100000 / (args.ipc_number*200))
...
def main_syn():
    ...
    for name in aux_teacher:
        if name == "resnet18":
            model = ResNet_cifar.resnet18(num_class=200)
        ...
        model_teacher[-1].load_state_dict(new_state_dict)
