...
# ResNet-32 for Tiny ImageNet
class ResNet_modify(nn.Module):
    def __init__(self, block, num_blocks, num_classes=200, nf=16):
        super(ResNet_modify, self).__init__()
        self.in_planes = nf

        # Input conv + maxpool for 64×64 image
        self.conv1 = nn.Conv2d(3, nf, kernel_size=3, stride=2, padding=1, bias=False)
        ...
        self.out_dim = nf * 4 * block.expansion
...
def resnet32(num_class=200):
    return ResNet_modify(BasicBlock_s, [5, 5, 5], num_classes=200, nf=16)
